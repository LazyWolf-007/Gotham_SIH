...
// Fallback Credentials for Network Failures / Offline Testing
const FALLBACK_ACCOUNTS: Record<string, { pass: string; user: UserProfile }> = {
  "admin@ncrb.gov.in": {
    ...
    user: {
      ...
      displayName: "Master Admin (NCRB HQ)",
      role: "admin",
  ...
  "investigator@ncrb.gov.in": {
    ...
    user: {
      ...
      displayName: "Inspr. Rajesh Sharma",
      role: "investigator",
  ...
  const login = async (emailInput: string, passInput: string) => {
    ...
    try {
      ...
      const profile: UserProfile = {
        ...
        displayName: cred.user.displayName || (role === "admin" ? "Master Admin" : "Investigator"),
        role,
      ...
      // If Firebase Cloud network request is blocked by AdBlocker/Firewall/VPN, auto-failover seamlessly
